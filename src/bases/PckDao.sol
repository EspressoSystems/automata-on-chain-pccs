...
abstract contract PckDao is DaoBase, SigVerifyBase {
    ...
    constructor(
        ...
        address _x509,
        address _crl
    ) SigVerifyBase(_p256, _x509) DaoBase(_resolver) {
        Pcs = PcsDao(_pcs);
    ...
    function PCK_KEY(
        ...
        key = keccak256(
            abi.encodePacked(PCK_MAGIC, qeidBytes, pceidBytes, tcbmBytes)
        );
    }

    function TCB_MAPPING_KEY(
        bytes16 qeid,
        bytes2 pceid,
    ...
    function getCert(
        ...
        bytes18 tcbmBytes = _tcbrToTcbmMapping(
            TCB_MAPPING_KEY(
                qeidBytes,
                pceidBytes,
                platformCpuSvnBytes,
        ...
        if (tcbmBytes != bytes18(0)) {
            pckCert = _onFetchDataFromResolver(
                PCK_KEY(qeidBytes, pceidBytes, tcbmBytes),
                false
            );
        }
    }
    ...
    function getCerts(
        string calldata qeid,
        string calldata pceid
    ) external view returns (string[] memory tcbms, bytes[] memory pckCerts) {
        (bytes16 qeidBytes, bytes2 pceidBytes, , , ) = _parseStringInputs(
            qeid,
            pceid,
            "",
            "",
        ...
        if (n > 0) {
            ...
            for (uint256 i = 0; i < n; ) {
                tcbms[i] = LibString.toHexStringNoPrefix(
                    abi.encodePacked(tcbmBytes[i])
                );
                pckCerts[i] = _onFetchDataFromResolver(
                    PCK_KEY(qeidBytes, pceidBytes, tcbmBytes[i]),
                    false
                );
    ...
    function upsertPckCert(
        ...
        (
            ...
            ,
            bytes18 tcbmBytes
        ) = _parseStringInputs(qeid, pceid, "", "", tcbm);
    ...
    function _validatePckTcb(
        ...
        bytes32 tcbmMappingKey = TCB_MAPPING_KEY(
            qeidBytes,
            pceidBytes,
            platformCpuSvnBytes,
            platformPceSvnBytes
    ...
    {
        intermediateCert = _onFetchDataFromResolver(
            Pcs.PCS_KEY(ca, false),
            false
        );
    ...
    function _attestPck(
        ...
        bytes32 hash,
        bytes32 key
    ) internal virtual returns (bytes32 attestationId) {
     ...
     */
    function _setTcbrToTcbmMapping(
        bytes32 tcbMappingKey,
        bytes18 tcbmBytes
    ) internal virtual;
     ...
     */
    function _tcbrToTcbmMapping(
        bytes32 tcbMappingKey
    ) internal view virtual returns (bytes18 tcbm);
     ...
     */
    function _getAllTcbs(
        bytes16 qeidBytes,
        bytes2 pceidBytes
    ) internal view virtual returns (bytes18[] memory tcbms);

    function _validatePck(
        CA ca,
        bytes memory der,
        bytes18 tcbm,
        bytes2 pceid
        ...
        bytes memory crlData = _fetchDataFromResolver(
            Pcs.PCS_KEY(ca, true),
            false
        );
        if (crlData.length > 0) {
            bool revocable = crlLib.serialNumberIsRevoked(
                pck.serialNumber,
                crlData
            );
            if (revocable) {
        ...
        bytes memory issuerCert = _fetchDataFromResolver(
            Pcs.PCS_KEY(ca, false),
            false
        );
        if (issuerCert.length > 0) {
            bytes32 digest = sha256(pck.tbs);
            bool sigVerified = verifySignature(
                digest,
                pck.signature,
                issuerCert
            );
    ...
    function _parseStringInputs(
        ...
        string memory tcbm
    ) internal pure returns (bytes16, bytes2, bytes16, bytes2, bytes18) {
        bytes16 qeidBytes;
        bytes2 pceidBytes;
        bytes16 platformCpuSvnBytes;
        bytes2 platformPceSvnBytes;
        ...
        if (bytes(platformCpuSvn).length == 32) {
            platformCpuSvnBytes = bytes16(
                uint128(_parseUintFromHex(platformCpuSvn))
            );
        }
        if (bytes(platformPceSvn).length == 4) {
            platformPceSvnBytes = bytes2(
                uint16(_parseUintFromHex(platformPceSvn))
            );
        }
